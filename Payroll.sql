...
-- when was the last hiring date
SELECT MAX(HireDate)
FROM employees

-- what was the total salary and bonus paid to each department
...
SELECT SUM(Bonus)/SUM(Salary) AS Bonus%
FROM employees

-- get the list of all employees that earns above 5000
SELECT * 
FROM employees
WHERE Salary > 5000
...
-- how many employees in Lagos state earn more than 250000
SELECT COUNT(*) 
FROM employees
WHERE Salary > 250000
AND State = 'Lagos'

-- get the list of employees that joined the company in 2023
SELECT *
FROM employee
